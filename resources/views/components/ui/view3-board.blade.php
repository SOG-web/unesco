        ...
        @if ($type == 'image')
            <div class="w-full flex flex-row items-start justify-start gap-4">
                ...
                <div class="flex flex-col items-start justify-start">
                    <h1 class="font-poppins font-semibold text-text-1 text-[12px]">
        ...
        @else
            <div class="w-full flex flex-row items-start justify-start gap-4">
                <div class="flex items-center justify-center w-[43px] h-[36px] bg-[#F3E4FF] rounded-[8px] px-1.5">
                    <p class="font-semibold text-center leading-[11.87px] text-text-1 text-[12px] text-wrap">
                        {{ $list['course'] }}</p>
                </div>
                <div class="flex flex-col items-start justify-start">
                    <h1 class="font-poppins font-semibold text-text-1 text-[12px]">
                        {{ $list['title'] . ' ' . $list['first_name'] . ' ' . $list['last_name'] }}</h1>
                    <p class="font-poppins font-normal text-text-2 text-[12px]">{{ $list['role'] }}</p>
                </div>
            </div>
        @endif
    @endforeach
