    ...
    <div
        ...
        <div class="w-full flex flex-wrap flex-row items-center justify-between py-[30px]" >
            <h1
                ...
                text-left text-text-1 lg:text-[22px] leading-[28px] lg:leading-[33px]" >Select students to assign</h1>
            {{--            search box--}}
            <div class="w-[217px] h-[41px] rounded-[10px] flex flex-row items-center justify-start gap-2 px-[10px] border-[2px] border-gray-4">
                ...
                <input
                    ...
                    x-on:input.debounce.0ms="$wire.filterStudents()"
                    name="search" type="text" placeholder="Search students"
        ...
        @foreach($filteredStudents as $student)
            <div wire:key="{{ $student->id }}" class="flex flex-row items-center justify-start gap-4">
                ...
                <div class="w-[43px] h-[43px] bg-[#F3E4FF] rounded-full">
                    <img src="{{ $student->profile_photo_url }}" alt="profile image" class="w-full h-full object-cover rounded-full">
                </div>
                <div class="flex flex-col items-start justify-start">
                    <h1 class="font-poppins font-semibold text-text-1 text-[12px] w-[150px] truncate">{{ $student->first_name. ' '. $student->last_name }}</h1>
                    <p class="font-poppins font-normal text-text-2 text-[12px] truncate w-[150px]">{{ $student->email }}</p>
                </div>
            </div>
        @endforeach
    </div>
