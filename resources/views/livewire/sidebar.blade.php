...
new class extends Component {
    ...
    public function mount($notices, $userRole)
    {
...
<div
    ...
    <div class="flex w-full flex-col max-w-[80%] items-center justify-start gap-3 px-[12px]">
        ...
        <div class="flex w-full flex-col items-center justify-start gap-1">
            <h1 class="text-[14px] 2xl:text-[18px] text-center font-medium font-poppins text-white">
                {{ auth()->user()->first_name }} {{ auth()->user()->last_name }}</h1>
            <p class="text-[12px] w-full text-center text-wrap font-light font-poppins text-[#cccccc] break-words">
                {{ auth()->user()->email }}</p>
        </div>
    </div>
    <div class="flex w-full flex-col items-center justify-start gap-3 pb-[30px]">
        ...
        <x-layouts.sidebar-link :href="$userRole == 'admin' ? '/students' : '/courses'" :active="$userRole == 'admin'
            ? str_contains(request()->fullUrl(), 'students')
            : str_contains(request()->fullUrl(), 'courses')">
            <x-icons.student :active="$userRole == 'admin'
                ? str_contains(request()->fullUrl(), 'students')
                : str_contains(request()->fullUrl(), 'courses')" />
            @if ($userRole == 'admin')
                <span>Students</span>
        ...
        <x-layouts.sidebar-link :href="$userRole == 'admin' ? '/courses' : '/assessments'" :active="$userRole == 'admin'
            ? str_contains(request()->fullUrl(), 'courses')
            : str_contains(request()->fullUrl(), 'assessments')">
            <x-icons.courses :active="$userRole == 'admin'
                ? str_contains(request()->fullUrl(), 'courses')
                : str_contains(request()->fullUrl(), 'assessments')" />
            @if ($userRole == 'admin')
                <span>Courses</span>
        ...
        <x-layouts.sidebar-link href="/notices" :active="str_contains(request()->fullUrl(), 'notices')">
        ...
        <x-layouts.sidebar-link :href="$userRole == 'admin' ? '/grades' : ($userRole == 'teacher' ? '/students' : '/grades')" :active="$userRole == 'admin'
            ? str_contains(request()->fullUrl(), 'teachers')
            : ($userRole == 'teacher'
                ? str_contains(request()->fullUrl(), 'students')
                : str_contains(request()->fullUrl(), 'grades'))">
            <x-icons.teachers :active="$userRole == 'admin'
                ? str_contains(request()->fullUrl(), 'teachers')
                : ($userRole == 'teacher'
                    ? str_contains(request()->fullUrl(), 'students')
                    : str_contains(request()->fullUrl(), 'grades'))" />
